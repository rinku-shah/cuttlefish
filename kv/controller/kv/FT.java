...
IRPCListener
{
	...
	//putArr of length 20
	static Long[] putArr = new Long[] {(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0, (long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0};
	
	static Long[] putArrAvg = new Long[] {(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0,(long)0};
	
	protected static ConcurrentHashMap<String,String> kv = new ConcurrentHashMap<String, String>(){ {

		}};
					
	public static void myStats() {
		String[] getStats1 = SyncManager.counterReceivedValues.toString().split(",");
		...
		String[] putStats1 = SyncManager.counterSentValues.toString().split(",");
		String[] putStats2 = putStats1[3].toString().split("=");
		String[] putStats3 = putStats2[1].toString().split("]");
		System.out.println("NUMBER of PUTS= " + putStats3[0].toString());
	
	}
		
	//migrate local hash map to synchronized stores
	public static void migrateLocalStore(ConcurrentHashMap<String, String> hm){
	...
	public static void printLocalStore(){
		...
		try {
			...
			System.out.println("PRINTING STOREEPC2");
			while(storeEPC2.entries().hasNext()){
				Entry<String, Versioned<String>> e = storeEPC2.entries().next();
				String k = e.getKey();
				String val = e.getValue().toString();
				System.out.println("key= "+ k + " Val = "+ val);
					
			}
			System.out.println("PRINTING STOREEPC3");
			while(storeEPC3.entries().hasNext()){
				Entry<String, Versioned<String>> e = storeEPC3.entries().next();
				String k = e.getKey();
				String val = e.getValue().toString();
				System.out.println("key= "+ k + " Val = "+ val);
					
			}
			System.out.println("PRINTING STOREEPC4");
			while(storeEPC4.entries().hasNext()){
				Entry<String, Versioned<String>> e = storeEPC4.entries().next();
				String k = e.getKey();
				String val = e.getValue().toString();
				System.out.println("key= "+ k + " Val = "+ val);
			}
			
			System.out.println("PRINTING STOREEPC5");
			while(storeEPC5.entries().hasNext()){
				Entry<String, Versioned<String>> e = storeEPC5.entries().next();
				String k = e.getKey();
				String val = e.getValue().toString();
				System.out.println("key= "+ k + " Val = "+ val);
					
			}
			System.out.println("PRINTING STOREEPC6");
			while(storeEPC6.entries().hasNext()){
				Entry<String, Versioned<String>> e = storeEPC6.entries().next();
				String k = e.getKey();
				String val = e.getValue().toString();
				System.out.println("key= "+ k + " Val = "+ val);
			}
	...
	public static void put(int msgId, DatapathId dgwDpId, String mapName, String key, String val){
		...
		if(Constants.CENTRALIZED && !Constants.MIGRATING){
			...
			if(mapName.equals("kv")){
				kv.put(k.toString(),val);
			}
		}
		else {
			try {
			    ...
			    //System.out.println("K= "+ k.toString() + "V= " + val);
			} catch (SyncException e) {
			    e.printStackTrace();
			}
		}
