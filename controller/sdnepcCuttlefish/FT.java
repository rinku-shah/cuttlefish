...
package net.floodlightcontroller.sdnepcSep_online;
...
//	private static ConcurrentHashMap<String, TransportPort> uekey_udp_src_port_map = new ConcurrentHashMap<String, TransportPort>(); // key = ue key, Value = UE UDP port number 
	...
	public static void migrateLocalStore(ConcurrentHashMap<String, String> hm){
		...
		try {
			//IStoreClient<String, String> tmpStore = getStoreFromDGW(dgwDpId);
			StringBuilder k = new StringBuilder();
			k.append(hm.toString()).append(Constants.MAPKEYSEPARATOR).append(key);
			storeEPC1.put(k.toString(), val);
			storeEPC2.put(k.toString(), val);
			storeEPC3.put(k.toString(), val);
			storeEPC4.put(k.toString(), val);
	...
	public static void put(DatapathId dgwDpId, String mapName, String key, String val){
		if(Constants.CENTRALIZED){
			if(mapName.equals("uekey_sgw_teid_map")){
				uekey_sgw_teid_map.put(key,val);
			}
			else if(mapName.equals("sgw_teid_uekey_map")){
				sgw_teid_uekey_map.put(key,val);
			}
			else if(mapName.equals("uekey_ueip_map")){
				uekey_ueip_map.put(key,val);
			}
			else if(mapName.equals("ue_state")){
				ue_state.put(key,val);
			}
			else if(mapName.equals("SGW_PGW_TEID_MAP")){
				SGW_PGW_TEID_MAP.put(key,val);
			}
		}
	...
	public static void del(DatapathId dgwDpId,String mapName, String key){
		if(Constants.CENTRALIZED){
			//String val="none";
			if(mapName.equals("uekey_sgw_teid_map")){
				uekey_sgw_teid_map.remove(key);
			}
			else if(mapName.equals("sgw_teid_uekey_map")){
				sgw_teid_uekey_map.remove(key);
			}
			else if(mapName.equals("uekey_ueip_map")){
				uekey_ueip_map.remove(key);
			}
			else if(mapName.equals("ue_state")){
				ue_state.remove(key);
			}
			else if(mapName.equals("SGW_PGW_TEID_MAP")){
				SGW_PGW_TEID_MAP.remove(key);
			}
		}
		else {	
		try {
