...
//import net.floodlightcontroller.sdnepc_remote.
...
public class MME implements IFloodlightModule, IOFMessageListener, IOFSwitchListener {
	...
	//private ConcurrentHashMap<String, String> uekey_sgw_teid_map = new ConcurrentHashMap<String, String>();//SYNC REMOTE
	//private ConcurrentHashMap<String, String> sgw_teid_uekey_map = new ConcurrentHashMap<String, String>();//SYNC REMOTE
	//private ConcurrentHashMap<String, String> uekey_ueip_map = new ConcurrentHashMap<String, String>();//SYNC REMOTE
	private ConcurrentHashMap<DatapathId, IOFSwitch> switch_mapping = new ConcurrentHashMap<DatapathId, IOFSwitch>();
	//private ConcurrentHashMap<String, Boolean> ue_state = new ConcurrentHashMap<String, Boolean>();	//SYNC REMOTE // Key: UE_Key, Value: State (TRUE: Active, FALSE: Idle)
	...
	//private ConcurrentHashMap<String, TransportPort> uekey_udp_src_port_map = new ConcurrentHashMap<String, TransportPort>();//SYNC REMOTE // key = ue key, Value = UE UDP port number 
	...
	public MME(){
		uePort = Constants.DEFAULT_SWITCH_UE_PORT;
		//OnlineDesignAdaptation Online = new OnlineDesignAdaptation();
	}
	...
	public Command receive(IOFSwitch sw, OFMessage msg, FloodlightContext cntx) {
		...
		case PACKET_IN:
			...
			/*++packetCount;
			//if(packetCount%10 == 0){
				FT.put(String.valueOf(packetCount), String.valueOf(packetCount));
			//}
			if(packetCount%10 ==0){
				FT.get(String.valueOf(packetCount));
				//System.out.println(FT.get(String.valueOf(packetCount)));
			}*/
			////////////////////////
			if(DatapathId.of(Constants.DEFAULT_SWITCH_ID_1).equals(sw.getId()) || DatapathId.of(Constants.DEFAULT_SWITCH_ID_2).equals(sw.getId()) || DatapathId.of(Constants.DEFAULT_SWITCH_ID_3).equals(sw.getId()) || DatapathId.of(Constants.DEFAULT_SWITCH_ID_4).equals(sw.getId())  || DatapathId.of(Constants.DEFAULT_SWITCH_ID_5).equals(sw.getId()) || DatapathId.of(Constants.DEFAULT_SWITCH_ID_6).equals(sw.getId())) {
				defaultSwitch = sw.getId(); //DatapathId.of(Constants.DEFAULT_SWITCH_ID); 
				/*++packetCount;
				if(packetCount%100 ==0)
					System.out.println("In Count:" + packetCount + "\n");*/
				try {
			...
			else if(DatapathId.of(Constants.SGW_ID_1).equals(sw.getId())|| DatapathId.of(Constants.SGW_ID_2).equals(sw.getId()) || DatapathId.of(Constants.SGW_ID_3).equals(sw.getId()) || DatapathId.of(Constants.SGW_ID_4).equals(sw.getId())  || DatapathId.of(Constants.SGW_ID_5).equals(sw.getId()) || DatapathId.of(Constants.SGW_ID_6).equals(sw.getId())) {
				/*++packetCount;
				if(packetCount%100 ==0)
					System.out.println("In Count:" + packetCount + "\n");*/
				try {
	...
	private Command processPacketInMessageFromSGW(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) throws NumberFormatException, InterruptedException {
		String payloadArray[];
	...
	private Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) throws InterruptedException {
		if (Constants.MIGRATE_HALT){ //Wait for ensuring ordered execution after switch
			try {
				...
				if(Data.class.isInstance(udpPkt.getPayload())){
					...
					if(payload.contains(Constants.SEPARATOR)){
						...
						case Constants.SEND_APN:
							...
							FT.put(Integer.parseInt(Constants.SEND_APN),dgw_dpId, "uekey_udp_src_port_map", tmpArray[2], srcPort.toString());
						...
						/*	if (defaultSwitch.equals(DatapathId.of(1)))
								sgw_dpid = DatapathId.of(Constants.SGW_ID_1);
							else if (defaultSwitch == DatapathId.of(5))
								sgw_dpid = DatapathId.of(Constants.SGW_ID_2);*/
							//sgw_dpid = DatapathId.of(Constants.SGW_ID_1);
							if(Constants.DEBUG){
								System.out.println("DEFAULT SWITCH installing uplink rule on default switch dpid = "+defaultSwitch.getLong()+" inport="+uePort+" and SRC IP = "+tmpArray2[0]+
										" outPort = "+Constants.ENODEB_SGW_PORT_MAP.get(defaultSwitch.getLong() + Constants.SEPARATOR + Constants.getSgwDpid(defaultSwitch))+" and out SRC IP = "+Constants.getRanIp(defaultSwitch)+" out teid= "+sgw_teid+" of UE key = "+tmpArray[2]);
							}
							//System.out.println(sgw_dpid + " ");
							...
							//uekey_sgw_teid_map.put(tmpArray[2], DatapathId.of(getSgwDpid(dgw_dpId)) + Constants.SEPARATOR + tmpArray2[1]);
							FT.put(Integer.parseInt(Constants.SEND_APN),dgw_dpId, "uekey_sgw_teid_map", tmpArray[2], DatapathId.of(Constants.getSgwDpid(dgw_dpId)) + Constants.SEPARATOR + tmpArray2[1]);
						...
						case Constants.SEND_UE_TEID:
							...
							sgw.modifyBearerRequest(switchService.getSwitch(DatapathId.of(tmpArray2[0])), DatapathId.of(tmpArray2[0]), Integer.parseInt(tmpArray2[1]), Integer.parseInt(tmpArray[1]), tmpArray[2]);
							...
							//////
							//ft.del(send_ue_teid_dgw,"uekey_ueip_map", tmpArray[2]);		// key is UE Key and value is UE IP
							/////
							/*
							if (defaultSwitch.equals(DatapathId.of(1)))
								sgw_dpid = DatapathId.of(Constants.SGW_ID_1);
							...
							//installFlowRule(DatapathId.of(getDgwDpid(DatapathId.of(tmpArray2[0]))), Constants.ENODEB_SGW_PORT_MAP.get(getDgwDpid(DatapathId.of(tmpArray2[0])) + Constants.SEPARATOR + DatapathId.of(tmpArray2[0]).getLong()), Integer.parseInt(tmpArray[1]), uePort, Integer.parseInt(tmpArray[1]), Constants.SINK_IP, ue_ip, Constants.UE_MAC);
							installFlowRule(DatapathId.of(Constants.getDgwDpid(DatapathId.of(tmpArray2[0]))), Constants.ENODEB_SGW_PORT_MAP.get(Constants.getDgwDpid(DatapathId.of(tmpArray2[0])) + Constants.SEPARATOR + DatapathId.of(tmpArray2[0]).getLong()), Integer.parseInt(tmpArray[1]), uePort, Integer.parseInt(tmpArray[1]), Constants.SINK_IP, ue_ip, Constants.getUeMac(send_ue_teid_dgw));
						...
						case Constants.DETACH_REQUEST:
							...
							/*if (defaultSwitch == DatapathId.of(1))
								sgw_dpid = DatapathId.of(Constants.SGW_ID_1);
							else if (defaultSwitch == DatapathId.of(5))
								sgw_dpid = DatapathId.of(Constants.SGW_ID_2);*/
						...
						case Constants.REQUEST_STARTING_IP:
							...
							String ip = sgw.getStartingIPAddress(sw);
							if(Constants.DEBUG){
						...
						case Constants.UE_CONTEXT_RELEASE_REQUEST:
							...
							DEFAULT_S_SGW_PORT = Constants.ENODEB_SGW_PORT_MAP.get(dw_cr.getLong() + Constants.SEPARATOR + Constants.getSgwDpid(dw_cr)); //dpids[0] ==> SGW DPID
							//delete downlink rule
							...
							// dpids[0] ==> SGW DPID   & dpids[1]==> PGW DPID 
							sgw.releaseAccessBearersRequest(switch_mapping.get(DatapathId.of(Constants.getSgwDpid(dw_cr))), DatapathId.of(Constants.getSgwDpid(dw_cr)), Integer.parseInt(tmpArray[3]), tmpArray[1]);
						...
						case Constants.UE_SERVICE_REQUEST:
							...
							if(Constants.DEBUG){
								System.out.println("DEFAULT SWITCH installing uplink rule on default switch dpid = "+dw_ue_ser.getLong()+" inport="+uePort+" and SRC IP = "+ue_ip+
										" outPort = "+Constants.ENODEB_SGW_PORT_MAP.get(dw_ue_ser.getLong() + Constants.SEPARATOR + Constants.getSgwDpid(dw_ue_ser))+" and out SRC IP = "+Constants.getRanIp(dw_ue_ser)+" out teid= "+sgw_teid+" of UE key = "+tmpArray[1]);
							}
						...
						case Constants.INITIAL_CONTEXT_SETUP_RESPONSE:
							DatapathId dw_c_resp = Constants.getDgwDpidFromIp(srcIp.toString());
							if(Constants.DEBUG){
							...
							/*String tmp_get = FT.get("uekey_sgw_teid_map", tmpArray[2]);
							while( tmp_get == null){
								tmp_get = FT.get("uekey_sgw_teid_map", tmpArray[2]);
							}*/
	...
	protected boolean downlinkDataNotification(String uekey, VlanVid vlan) throws NumberFormatException, InterruptedException{
		/*TransportPort srcPort = TransportPort.of(Constants.DEFAULT_CONTROL_TRAFFIC_UDP_PORT), dstPort = TransportPort.of(67);
		IPv4Address srcIp = IPv4Address.of(getDgwIp(defaultSwitch)), dstIp = IPv4Address.of(getRanIp(defaultSwitch));
		MacAddress srcMac = MacAddress.of(Constants.DEFAULT_GW_MAC), dstMac =  MacAddress.of(Constants.UE_MAC);
		IOFSwitch sw = switchService.getSwitch(defaultSwitch);
		...
		dstPort = TransportPort.of(Integer.parseInt(ft.get(defaultSwitch, "uekey_udp_src_port_map", uekey)));
		if(dstPort == null){
			System.out.println("ERROR: UE UDP port not found for the UE key = " + uekey);
			//System.exit(1);
		}
	...
	public void switchAdded(DatapathId switchId) {
		...
		//if (!(switchId.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID)))){
			
			// Install ARP rule
		    //OFActionNiciraDecTtl t;
			OFFlowMod.Builder fmb = sw.getOFFactory().buildFlowAdd();
			Match.Builder mb = sw.getOFFactory().buildMatch();
...
/////////////////////////////////// CODE TO DIVERT FLOWS TO SPECIFIC CONTROLLER /////////////////////////////////////////
			...
			switchAddedCount++;
			/*if (flag==false && switchAddedCount == 13){
				flag = true;*/
			/*if(flag){
				CuttleThread Cut = new CuttleThread();
				flag = false;
			}*/
			if (switchAddedCount == 13){
				...
				Thread t1 = new Thread(Online) ;
				Runnable Cut = new CutThread();
				Thread t2 = new Thread(Cut);
				
				t1.start();
				t2.start();*/
				
				//OnlineDesignAdaptation Online = new OnlineDesignAdaptation(sw1,sw2,sw3,sw4,sw5,sw6);
				/*CuttleThread Cut = new CuttleThread();*/
			...
			//sudo ovs-ofctl add-flow br0 ip,in_port=4,dl_src=00:16:3e:1f:c0:12,actions=mod_nw_dst:10.128.41.1,CONTROLLER:max_len=65535,CONTROLLER:id=2
		//sudo ovs-ofctl add-flow br0 tcp,in_port=4,dl_src=00:16:3e:1f:c0:12,actions=mod_nw_dst:10.128.41.1,output=6
	...
	public void initCentralizedDesign(DatapathId switchId, IOFSwitch sw){
		...
		Constants.CENTRALIZED = true; 
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_1)))
			{
				...
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,20);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,5);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,9);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,12);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,13);
				//controllerId = Constants.CONTROLLER_ID_LOCAL_1;
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,7);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,14);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,17);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,19);
			}
	...
	public void initRemoteDesign(DatapathId switchId, IOFSwitch sw){
		...
		if (Constants.OFFLOAD_ATTACH){
			...
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_2)))
			{
				...
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,12);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,13);
				controllerId = Constants.CONTROLLER_ID_LOCAL_2;
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,7);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,14);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,17);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,19);
			...
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_3)))
			{
				...
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,12);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,13);
				controllerId = Constants.CONTROLLER_ID_LOCAL_3;
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,7);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,14);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,17);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,19);
			...
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_4)))
			{
				...
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,12);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,13);
				controllerId = Constants.CONTROLLER_ID_LOCAL_4;
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,7);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,14);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,17);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,19);			
			...
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_5)))
			{
				...
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,12);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,13);
				controllerId = Constants.CONTROLLER_ID_LOCAL_5;
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,7);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,14);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,17);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,19);
			...
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_6)))
			{
				...
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,12);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,13);
				controllerId = Constants.CONTROLLER_ID_LOCAL_6;
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,7);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,14);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,17);
				addDefaultSwitchNetRuleController(sw_t,ranIp,controllerId,19);
		...
		else //Attach request is not offloaded
		{
			if (switchId_t.equals(DatapathId.of(Constants.DEFAULT_SWITCH_ID_1)))
			{
				String ranIp = Constants.RAN_IP_1;
	...
	public void addDefaultSwitchNetRule(IOFSwitch sw, int msg_id) {
		...
		mb.setExact(MatchField.IN_PORT,OFPort.of(4));
		mb.setExact(MatchField.ETH_TYPE, EthType.IPv4);
		//mb.setExact(MatchField.IPV4_DST,IPv4Address.of("10.127.41.3"));
		mb.setExact(MatchField.ETH_SRC,MacAddress.of("00:16:3e:1f:c0:12"));
		//mb.setExact(MatchField.IP_DSCP,IpDscp.of((byte)ip_tos));
		//mb.setWildcards(Wildcards.FULL.withNwSrcMask(24).withNwDstMask(8));
	
		//OFActionOutput.Builder actionBuilder = sw.getOFFactory().actions().buildOutput();
		ArrayList<OFAction> actionList = new ArrayList<OFAction>();
		
		OFActionSetNwDst action1 = sw.getOFFactory().actions().setNwDst(IPv4Address.of("10.128.41.1"));
		//OFActionSetDlDst action2 = sw.getOFFactory().actions().setDlDst(MacAddress.of("00:16:3e:ee:a0:5f"));
		//OFActionOutput action3 = sw.getOFFactory().actions().output(OFPort.CONTROLLER,0);
		...
		OFActionOutput action5 = sw.getOFFactory().actions().buildOutput()
		        ...
		        //.setMaxLen(0xffFFffFF)
		        .build();
		//OFActionSetTpDst action4 = sw.getOFFactory().actions().setTpDst(TransportPort.of(6653));
		//sw.getOFFactory().actions().setDlDst(MacAddress.of("00:16:3e:c6:64:57"));
	    //actionBuilder.setPort(OFPort.of(1));
		...
		//fmb.setActions(Collections.singletonList((OFAction) actionBuilder.build()));
		//OFActionOutput.Builder actionBuilder = sw.getOFFactory().actions().buildOutput();
		//actionBuilder.setPort(OFPort.NORMAL); //NORMAL);		
		fmb.setHardTimeout(0)
		.setIdleTimeout(0)
		.setPriority(1)
		...
		/*
		////////////////////    CODE TO DROP PACKETS ON TOS MATCH /////////////////////
		mb.setExact(MatchField.IN_PORT,OFPort.of(4));
		mb.setExact(MatchField.ETH_TYPE, EthType.IPv4);
		mb.setExact(MatchField.ETH_SRC,MacAddress.of("00:16:3e:1f:c0:12"));
		mb.setExact(MatchField.IP_DSCP,IpDscp.of((byte)12));

		ArrayList<OFAction> actionList1 = new ArrayList<OFAction>();

		OFActionOutput action9 = sw.getOFFactory().actions().buildOutput()
				.setPort(OFPort.ZERO)
		        .build();

		actionList.add(action9);
		fmb.setActions(actionList1);	
		fmb.setHardTimeout(0)
		.setIdleTimeout(0)
		.setPriority(1)
		.setBufferId(OFBufferId.NO_BUFFER)
