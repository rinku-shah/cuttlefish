...
public class PGW implements IFloodlightModule {
	...
	*/
	...
	public String returnStartingIPAddress(IOFSwitch sw){
		DatapathId dp = sw.getId();
		
		return ip[Constants.getChainIDFromDGW(dp)];
	}
	...
	public synchronized String allocateIPForUE(IOFSwitch pgw, int sgw_tunnelId, DatapathId sgw_dpId, DatapathId pgw_dpId, String apn){
		...
		if(Constants.DEBUG)
			System.out.println("ALLOCATE IP for chain "+chainId+" sgw teid "+ sgw_tunnelId + " sgwdpid " + sgw_dpId);
		...
		long dgw_dp_id_temp = Constants.getDgwDpid(sgw_dpId);
		String sgw_ip = Constants.getSgwIpDownlink(DatapathId.of(dgw_dp_id_temp));
	...
	private synchronized void installFlowRule(IOFSwitch sw, DatapathId dpId, int inPort, int inTunnelId, int outPort, int outTunnelId, String srcIP, String dstIP){
		...
		boolean x = sw.write(fmb.build());
		if(!x){
			System.out.println("PGW install failed");
			System.exit(0);
		}
