...
public class SGW implements IFloodlightModule, IOFMessageListener {
	...
	String contactPGW(IOFSwitch sgw, IOFSwitch pgw, DatapathId sgw_dpId, DatapathId pgw_dpId, String apn){
		long dgwId = Constants.getDgwDpid(sgw_dpId);
		...
		FT.put(Integer.parseInt(Constants.SEND_APN), DatapathId.of(dgwId), "SGW_PGW_TEID_MAP", String.valueOf(sgw_teid), String.valueOf(pgw_teid));
	...
	public boolean detachUEFromSGW(IOFSwitch sgw, IOFSwitch pgw, DatapathId sgw_dpId, DatapathId pgw_dpId, int sgw_teid, String ue_ip) throws NumberFormatException, InterruptedException{
		...
		FT.del(Integer.parseInt(Constants.DETACH_REQUEST),dgwId, "SGW_PGW_TEID_MAP", String.valueOf(sgw_teid));
	...
	public void releaseAccessBearersRequest(IOFSwitch sgw, DatapathId sgw_dpId, int sgw_teid, String ue_ip){
		//delete downlink rule
		//System.out.println("sgw " + sgw + "sgw dpid " + sgw_dpId + "sgwteid " + sgw_teid + "ue_ip " + ue_ip);
		deleteFlowRuleWithTEID(sgw, Constants.SGW_PORT_MAP.get(sgw_dpId)[1], sgw_teid, Constants.SINK_IP);
		if(Constants.DEBUG){
			System.out.println("SGW deleting downlink rule with SGW TEID="+sgw_teid+" for UE with IP="+ue_ip);
		}
